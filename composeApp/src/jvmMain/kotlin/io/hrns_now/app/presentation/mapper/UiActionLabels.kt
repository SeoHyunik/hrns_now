...
fun UiAction.displayLabel(locale: AppLocale = AppLocale.Korean): String =
    when (locale) {
        ...
        AppLocale.English -> when (this) {
            UiAction.ConnectProject -> "Connect project"
            UiAction.SelectWorkspaceDay -> "Select date"
            UiAction.OpenToday -> "Go to today"
            UiAction.Refresh -> "Refresh"
            UiAction.EditRequest -> "Write request"
            UiAction.RunDoctor -> "Check connection"
            UiAction.RunOpsValidation -> "Check readiness"
            UiAction.BootstrapDay -> "Start today's work"
            UiAction.RunPlanning -> "Run planning"
            UiAction.RunReplan -> "Run replanning"
            UiAction.ReviewPlan -> "Review plan"
            UiAction.RunCodeSlice -> "Run selected code task"
            UiAction.RunDocSlice -> "Run selected doc task"
            UiAction.RunValidationSlice -> "Run validation-only task"
            UiAction.RunClosureValidation -> "Run closure validation"
            UiAction.ReviewClosure -> "Review closure"
            UiAction.CloseDay -> "Close today"
            UiAction.OpenRecoveryCenter -> "Open recovery center"
            UiAction.ShowCompatibilityIssue -> "Check compatibility"
            UiAction.ViewExecutionStatus -> "View run history"
        }
    }

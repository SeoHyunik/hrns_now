 ...
 *
 * `workspaceConfig`는 더 이상 생성 시점에 고정하지 않는다 — 다중 프로젝트 전환을
 * 지원하려면 호출마다 활성 프로젝트의 config를 명시적으로 받아야 한다. 함수형 의존성은 기존 단일 메서드 probe를 위한 좁은 port
...
class LoadCockpitUseCase(
    ...
    fun resolveDays(
        ...
        // 오늘은 daily directory discovery 결과와 무관하게 명시 선택을 허용한다 —

...
/**
 * 사용자가 Setup 화면에서 입력한, 아직 검증되지 않은 신규 프로젝트 후보다. `useDefaultKit`가
 * true면 [kitRootRaw]는 무시한다 — 표준 등록 흐름은 Kit 경로를 전혀 입력받지 않는다.
 * `kitRootRaw`는 사용자가 고급 설정에서 명시적으로 `외부 Harness Kit 사용`을 선택했을 때만 쓰인다.
 */
data class RegisterProjectCandidate(
    val displayName: String,
    val useDefaultKit: Boolean,
...
/**
 * `InvalidCandidate`의 typed 원인이다 — presentation 계층이 사람이 읽는
 ...
 * 않지만(특히 `DefaultKit`), 등록 전 Doctor gate처럼 즉시 실행이 필요한 호출자는 이
 ...
 * 사이에 이를 우회하는 경로를 만들지 않는다는 설계 원칙을 지킨다. 실제 경로 실재/real path 확인은
 * [pathResolver](주입된 `infra.registry.RealPathGateway`)가 담당하고, runtime source
 * 가용성/무결성 확인은 [runtimeSourceResolver](주입된 `infra.runtime.DefaultKitRuntimeResolver`)가
 * 담당한다. 이 use case와 [BoundaryPolicy]는 그 결과만 소비한다.
 */
class RegisterProjectUseCase(
    ...
    /**
     * 후보를 저장하지 않고 경계까지 검증한다. Onboarding은 이 결과로 Doctor와
    ...
    fun inspect(candidate: RegisterProjectCandidate): ProjectRegistrationInspection {
        ...
        val runtimeSourceCandidate: RuntimeSource = if (candidate.useDefaultKit) {
    ...
    private fun missingRuntimeMessage(source: RuntimeSource): String =
        when (source) {
            RuntimeSource.DefaultKit ->
                "기본 Harness Kit(.local\\harness-kit)을 찾을 수 없습니다. Kit을 준비하거나 고급 설정에서 외부 Harness Kit을 선택하세요."

...
작업 시작 전 `git log -1 --oneline`으로 현재 `harness-dev` HEAD를 기록한다. 이 Phase의 다음 선행 커밋이 history에 포함되어야 한다.

```text
34f57dc docs: Phase 7 내장 개발 SDK 작업 정의
```

위 커밋은 Phase 6 UI/UX 보정 커밋 `b8ee279 fix: Phase 6 UI UX 피드백과 용어 개선`을 포함한 후속 커밋이다. 그 뒤의 Codex 문서 커밋은 보존하고, 어떤 Git 변경도 수행하지 않는다.
